...
export default class DataExporter extends Base{
    ...
    constructor(config:DataExporterConfigType){
       ...
       this._downloadBtn = this._creatDownloadBtn(this._getFilename());
        ...
        // 添加表头
       this._data.push(['笔记标题', '笔记链接', '作者', '作者链接', '点赞数', '是否违规']);
       // 第一次加载，提取笔记内容数据
    ...
    private _extractOneNote = (ele:Element)=>{
        ...
        const noteLink = titleElement ? 'https://www.xiaohongshu.com' + ele.querySelector('a')?.getAttribute('href'):'';
